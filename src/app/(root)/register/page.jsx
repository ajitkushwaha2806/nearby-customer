...
export default function RegisterPage() {
    ...
    const formik = useFormik({
        ...
        onSubmit: async (values) => {
            ...
            toast.promise(AuthService.register({ name: values.name, phone: values.phone, password: values.password }), {
                loading: "Creating your account...",
                success: (data) => {
                    setTimeout(() => router.push("/"), 500);
                    return `Welcome to the platform, ${data?.user?.name || values.name}!`;
                },
                error: (err) => {
                    setIsLoading(false);
